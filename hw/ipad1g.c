...
{
	s5l8930_state *cpu;
 	uint32_t llb_size, llb_filesize, iboot_size, iboot_filesize, vrom_size;
	ram_addr_t phys_flash, ramoff;
	target_phys_addr_t llb_base = LLB_LOAD_ADDR;
	target_phys_addr_t iboot_base = IBOOT_LOAD_ADDR;
	//target_phys_addr_t llb_base = 0;
	target_phys_addr_t vrom_base  = SROM_LOAD_ADDR;
	...
	//llb_size = get_image_size(option_rom[1].name);
	
	vrom_size = get_image_size(option_rom[0].name);
    llb_filesize = get_image_size(option_rom[1].name);
	//iboot_size = get_image_size(option_rom[2].name);
	iboot_filesize = get_image_size(option_rom[2].name);
	iboot_size = 0x100000;
	//vrom_size = 0;
    ...
    if(vrom_size != 0)
    {
        ...
        load_image_targphys(option_rom[0].name, vrom_base, vrom_size);
		printf("Loaded vmrom @ 0x%08x with size %d\n", vrom_base, vrom_size);
    } else {
        printf("No vrom assuming OIB\n");
		llb_base = 0x0;
		llb_filesize = get_image_size(option_rom[1].name);
		printf("Loaded LLB @ 0x%08x with fakesize %d and realsize %d\n", llb_base, llb_size, llb_filesize);
    }

	if(llb_filesize == -1 || llb_filesize == 0)
	{
		printf("A valid llb image is required | openiboot may be supplied with -option-rom\n");
		exit(1);
  	}
	
	if(vrom_size == -1)
	{
		printf("A valid vrom image is required | supply it with -option-rom\n");
		exit(1);
	}
	
	cpu_register_physical_memory(llb_base, (ram_addr_t)llb_size,(phys_flash = qemu_ram_alloc(NULL, "ipad1g.llb", llb_size)));
	load_image_targphys(option_rom[1].name, llb_base, llb_size);
	
	
	if(iboot_size == -1 || iboot_size == 0)
	{
		printf("A valid iBoot image is required | supply it with -option-rom\n");
	} else {
		cpu_register_physical_memory(iboot_base, (ram_addr_t)iboot_size,(phys_flash = qemu_ram_alloc(NULL, "ipad1g.iboot", iboot_size)));
		load_image_targphys(option_rom[2].name, iboot_base, iboot_size);
		//TODO: Fixme
		/* it appears that loading iboot at the *proper* iboot load addr breaks something..? */
		/* loading it at the LLB loading addr works though */
	}
	...
	printf("Loaded LLB @ 0x%08x with fakesize %d and realsize %d\n", llb_base, llb_size, llb_filesize);
	printf("Loaded iBoot @ 0x%08x with fakesize %d and realsize %d\n", iboot_base, iboot_size, iboot_filesize);
